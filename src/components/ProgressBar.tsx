    ...
    <div className="w-full max-w-3xl mx-auto mb-16 px-4">
      <div className="relative">
        <div className="h-1 w-full bg-gray-200 rounded-full absolute top-5">
          <div 
            className="h-1 bg-gold rounded-full transition-all duration-300 ease-in-out"
        ...
        <div className="flex justify-between relative">
          {[...Array(totalSteps)].map((_, index) => (
            <div key={index} className="flex flex-col items-center z-10">
              ...
              <div className={`text-sm font-medium text-center ${index === currentStep - 1 ? 'text-purple' : 'text-gray-500'} text-balance`}>
                {stepLabels[index]}
              </div>
            </div>
          ))}
